...
| **得られること** | `/security-review` の設計思想を理解し、自分のツール設計に応用できるようになる |
| **前提知識** | PRレビューの経験があれば十分。セキュリティの専門知識は不要 |

### 記事の構成

この記事は、**シンプル版 → 詳細版**の順で、どちらも「表 → フローチャート → 説明」という3段で理解できるように構成している。

> **シンプル版**：まず全体像を掴みたい人向け（5分）  
> **詳細版**：自チームで実装・カスタマイズしたい人向け（10分）
...
**具体例：Step5-2で除外されるケース**

```javascript
// この差分は「環境変数依存」としてStep5-2で除外される
...
環境変数から取得した値をそのまま使っているコードは、一見すると「シークレットのハードコード」と誤検知されそうだが、PRECEDENTSで「環境変数は信頼値として扱う」と定義しているため自動的に除外される。
...
詳細版のフローは情報量が多いため、3つに分割して示す。

#### Part 1: 入力固定〜一次抽出（Step1〜Step3）
...
#### Part 2: 除外フィルタリング（Step4〜Step5）

```mermaid
flowchart TD
    PREV["Part 1から →"] --> FORK2["Step4 並列開始：HARD EXCLUSIONS 評価"]
    ...
    JOIN3 -->|"該当あり"| X2["除外"]
    JOIN3 -->|"該当なし"| NEXT2["→ Part 3へ"]
```

#### Part 3: 確信度評価〜最終出力（Step6〜Step7）

```mermaid
flowchart TD
    PREV2["Part 2から →"] --> CONF1
...
2. **カスタマイズする**：Step4/Step5のルールを自チームのスタック（使用フレームワーク、セキュリティポリシー）に合わせて調整する
...
| 用語 | 説明 |
|---|---|
| **SQLi** | SQL Injection。SQLクエリに悪意のある入力を注入する攻撃 |
| **CMDi** | Command Injection。OSコマンドに悪意のある入力を注入する攻撃 |
| **XXE** | XML External Entity。XMLパーサーの外部エンティティ参照を悪用する攻撃 |
| **RCE** | Remote Code Execution。リモートから任意のコードを実行させる攻撃 |
| **Auth/AuthZ** | Authentication（認証）/ Authorization（認可）。誰であるかの確認と、何ができるかの制御 |
| **SSRF** | Server-Side Request Forgery。サーバーに意図しないリクエストを送信させる攻撃 |
| **DoS** | Denial of Service。サービスを利用不能にする攻撃 |
| **XSS** | Cross-Site Scripting。Webページに悪意のあるスクリプトを埋め込む攻撃 |
